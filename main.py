    ...
    def ticktock(self):
        ...
        if mwir_camera is not None:
            ...
            if mwir_frame is not None:
                ...
                self.last_mwir_frame = cv2.cvtColor(mwir_frame, cv2.COLOR_BGR2RGB)

                # Scale 
                scaled_mwir_frame = cv2.resize(self.last_mwir_frame, self.video_display_size, interpolation = cv2.INTER_NEAREST)
                #scaled_mwir_frame = self.last_mwir_frame
            ...
            # Overlay image?
            if mwir_got_new_frame and self.overlay:
                """
                overlay_image = np.zeros_like(self.last_mwir_frame)

                overlay_image[:,:,2] = 0.125 * self.last_mwir_frame[:,:,0] + 0.25 * self.last_mwir_frame[:,:,1] + 0.125 * self.last_mwir_frame[:,:,2]
                ...
                self.mwir_cam_image = ImageTk.PhotoImage(image=Image.fromarray(overlay_image))
                self.mwir_cam_panel.configure(image=self.mwir_cam_image)
                """
                pass
