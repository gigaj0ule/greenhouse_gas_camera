...
mwir_camera_starting_dev_index = 2
mwir_camera_serial_port = "/dev/ttyUSB0"
mwir_camera_type = 1

# Disable camera to run program without camera
use_camera = True
...
if use_camera is not False:
    ...
    while mwir_camera_starting_dev_index < 10:
        mwir_camera = None
        try:
            # Open camera at index
            mwir_camera = cv2.VideoCapture(int(mwir_camera_starting_dev_index), cv2.CAP_V4L2)
            break
        except:
            # No success? Increment index
            mwir_camera_starting_dev_index += 1
            print("Could not open video stream {}".format(mwir_camera_starting_dev_index))
else:
    mwir_camera = None
...
if mwir_camera is not None:
    ...
    os.system("v4l2-ctl -d /dev/video"+str(mwir_camera_starting_dev_index)+" -i 0 -s 0 --set-fmt-video=width=320,height=240,pixelformat=4")
...
class DualCamera():
    def __init__(self):
        ...
        # Camera Params
        self.agc_mode = 0
        self.agc_mode_strings = ["Off", "Combo", "HistEQ", "Linear", "HstFull", "HistPad"]
        self.agc_shift = 0
        self.focus_servo = 0
        self.autocal_mode = 0
        self.pallete = 0
        self.pallete_strings = ["WhtHot", "BlkHot", "Ironbow", "InvIron", "Rainbow", "InvRain", "Ocean", "InvOcean", "Sepia"]
        self.day_mode = 0
        
        # Videos
        self.mwir_cam_image = ImageTk.PhotoImage(image=Image.fromarray(np.uint8(256 * np.random.rand(240, 320))))
        self.mwir_cam_panel = tk.Label(image = self.mwir_cam_image)
        self.mwir_cam_panel.grid(row = 0, rowspan=8, column = 6, columnspan=3)
        ...
        self.autocal_button = tk.Button(width = 10, height = 2, text = 'Autocal: ?', command=self.SET_cam_autocal)
        self.autocal_button.grid(row = row_calibration, column = 2)
        ...
        self.day_mode_button = tk.Button(width = 10, height = 2, text = 'DayMode: ?', command=self.SET_cam_day_mode)
        self.day_mode_button.grid(row = row_sensitivity, column = 3)
        ...
        self.fov_n_button = tk.Button(width = 10, height = 2, text = 'FOV 2.7°', command=self.SET_cam_fovn)
        self.fov_n_button.grid(row = row_lens, column = 3)

        self.fov_w_button = tk.Button(width = 10, height = 2, text = 'FOV 9.1°', command=self.SET_cam_fovm)
        self.fov_w_button.grid(row = row_lens, column = 2)

        self.fov_w_button = tk.Button(width = 10, height = 2, text = 'FOV 31.5°', command=self.SET_cam_fovw)
        self.fov_w_button.grid(row = row_lens, column = 1)
        ...
        row_focus_2 = row_focus + 1
        self.image_label = tk.Label(self.root, text="Image")
        self.image_label.grid(row = row_focus_2, column = 0)

        self.pallete_button = tk.Button(width = 10, height = 2, text = 'Palette', command=self.SET_cam_pallete)
        self.pallete_button.grid(row = row_focus_2, column = 1)

        #self.focus_far_button = tk.Button(width = 10, height = 2, text = 'FOCUS Far', command=self.SET_cam_focusin)
        #self.focus_far_button.grid(row = row_focus_2, column = 2)

        #self.focus_near_button = tk.Button(width = 10, height = 2, text = 'FOCUS Near', command=self.SET_cam_focusnear)
        #self.focus_near_button.grid(row = row_focus_2, column = 3)


        # AGC
        row_agc = row_focus_2 + 1

        self.agc_label = tk.Label(self.root, text="A.G.C.")
        self.agc_label.grid(row = row_agc, column = 0)

        self.agc_mode_button = tk.Button(width = 10, height = 2, text = 'Mode: ?', command=self.SET_cam_agc_mode)
        self.agc_mode_button.grid(row = row_agc, column = 1)
        ...
        self.agc_plus_button = tk.Button(width = 10, height = 2, text = 'AGC +', command=self.SET_cam_agc_plus)
        self.agc_plus_button.grid(row = row_gain, column = 1)

        self.agc_minus_button = tk.Button(width = 10, height = 2, text = 'AGC -', command=self.SET_cam_agc_minus)
        self.agc_minus_button.grid(row = row_gain, column = 2)

        self.mwir_camera_gain = tk.IntVar()
        self.gain_slider = tk.Scale(self.root, from_=0, to=4096, length=800, variable=self.mwir_camera_gain, orient=tk.HORIZONTAL, command=self.SET_gain)
        self.gain_slider.grid(row = row_gain, rowspan = 1, column = 3, columnspan=9)
        ...
        self.agc_bias_up_button = tk.Button(width = 10, height = 2, text = 'AGC +', command=self.SET_cam_agc_bias_up)
        self.agc_bias_up_button.grid(row = row_bias, column = 1)

        self.agc_bias_down_button = tk.Button(width = 10, height = 2, text = 'AGC -', command=self.SET_cam_agc_bias_down)
        self.agc_bias_down_button.grid(row = row_bias, column = 2)
  
        self.mwir_camera_bias = tk.IntVar()
        self.bias_slider = tk.Scale(self.root, from_=0, to = 4096, length = 800, variable=self.mwir_camera_bias, orient=tk.HORIZONTAL, command=self.SET_bias)
        self.bias_slider.grid(row = row_bias, rowspan =1, column = 3, columnspan=9)


        # Overlay slider
        row_overlay_saturation = row_bias + 1
        self.mwirt_camera_saturation = tk.IntVar()
        self.saturation_label = tk.Label(self.root, text="Motion")
        self.saturation_label.grid(row=row_overlay_saturation, column = 0)
                
        self.overlay_button = tk.Button(width=10, height=2, text='Show', command=self.toggle_overlay)
        self.overlay_button.grid(row = row_overlay_saturation, column = 1)
        
        self.saturation_slider = tk.Scale(self.root, from_=0, to=10, length=800, variable=self.SATURATION, orient=tk.HORIZONTAL, command=self.SET_saturation)
        self.saturation_slider.grid(row = row_overlay_saturation, rowspan =1, column = 3, columnspan=9)
        ...
        self.filter_k_slider = tk.Scale(self.root, from_=1, to=10, length=800, variable=self.FILTER_K, orient=tk.HORIZONTAL, command=self.SET_filter_k)
        self.filter_k_slider.grid(row = row_filter_k, rowspan=1, column = 3, columnspan = 9)
        ...
        if mwir_camera is not None:
            ...
            #mwir_frame = cv2.ze(mwir_frame, (320, 240))
    ...
    # Detect when window is closed and exit
    def on_closing(self):
        self.root.destroy()
    ...
    def ticktock(self):
        ...
        if mwir_camera is not None:
            ...
            if mwir_frame is not None:
                self.mwir_rgb_frame = cv2.cvtColor(mwir_frame, cv2.COLOR_BGR2RGB)
            
                #frame = imutils.resize(frame, height=240)
                #mwir_frame = cv2.resize(mwir_frame, (320, 240))
                ...
                reference_channel = new_mwir_frame_int
                ...
                if self.overlay:
                    ...
                    window_preview_frame = self.mwir_rgb_frame.astype(np.uint8)
            ...
            if self.is_recording_signal.is_set():
                self.ir_frames.put([self.mwir_rgb_frame, self.new_post_processed_mwir_frame])

            # Is this a snapshot?
            if self.snapshot is True:
                self.snapshot = False
                print("Taking snapshot...")
                self.snapshot_frames.put([self.mwir_rgb_frame, self.new_post_processed_mwir_frame])
    ...
    def SET_cam_agc_plus(self):
        cmd = "AGC+:"
        mwir_camera_cmd.camCmd(cmd)


    def SET_cam_agc_minus(self):
        cmd = "AGC-:"
        mwir_camera_cmd.camCmd(cmd)


    def SET_cam_agc_bias_up(self):
        self.SET_agc_shift(0)


    def SET_cam_agc_bias_down(self):
        self.SET_agc_shift(1)
        

    def SET_agc_shift(self, direction):
        ...
        if self.agc_shift > 400:
            self.agc_shift = 400
            
        if self.agc_shift < 0:
            self.agc_shift = 0
                
        cmd = f"AGCSHIFT:{self.agc_shift}"
        mwir_camera_cmd.camCmd(cmd)                


    def SET_cam_pallete(self):
        self.pallete += 1
        if self.pallete > 8:
            self.pallete = 0

        self.pallete_button.configure(text = f"{self.pallete_strings[self.pallete]}")

        cmd = f"COLOR:1"
        mwir_camera_cmd.camCmd(cmd)        
                    
        cmd = f"PALETTE:{self.pallete}"
        mwir_camera_cmd.camCmd(cmd)


    def SET_cam_agc_mode(self):
        self.agc_mode += 1
        if self.agc_mode > 5:
            self.agc_mode = 0
            
        cmd = f"AGCMODE:{self.agc_mode}"
        
        self.agc_mode_button.configure(text = f"Mode: {self.agc_mode_strings[self.agc_mode]}")
        mwir_camera_cmd.camCmd(cmd)
    ...
    def SET_cam_focusservo(self, mode):   
        cmd = f"FOCUSSERVO:{mode}"
        mwir_camera_cmd.camCmd(cmd)


    def SET_cam_autocal(self):
        if self.autocal_mode == 0:
            self.autocal_mode = 1
            cmd = "AUTOCALON:"
            self.autocal_button.configure(text = f"Autocal: ON")
        else:
            self.autocal_mode = 0
            cmd = "AUTOCALOFF:"
            self.autocal_button.configure(text = f"Autocal: OFF")
                        
        mwir_camera_cmd.camCmd(cmd)


    def SET_cam_day_mode(self):
        if self.day_mode == 0:
            self.day_mode = 1
            cmd = "DAYMODE:1"
            self.day_mode_button.configure(text = f"DayMode: ON")
        else:
            self.day_mode = 0
            cmd = "DAYMODE:0"
            self.day_mode_button.configure(text = f"DayMode: OFF")
    ...
    def reset_agc_mode(self):
        self.agc_mode = 0
        self.agc_mode_button.configure(text = f"Mode: {self.agc_mode_strings[0]}")
    ...
    def toggle_overlay(self):
        if self.overlay:
            self.overlay = False
            self.overlay_button.configure(text = 'Show')
        else:
            self.overlay = True
            self.overlay_button.configure(text = 'Hide')
